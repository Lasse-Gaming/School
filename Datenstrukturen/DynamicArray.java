...
public class DynamicArray {
    ...
    public void remove(Object o) {
        int positionOfO = -1;
        for (int i=0; i<data.length; i++) {
            if (data[i] == o) positionOfO = i; 
        } 

        if (positionOfO >= 0) {
            Object[] new_data = new Object[data.length-1];
            for (int i=0; i<data.length; i++) {
                if (i<positionOfO) new_data[i] = data[i];
                if (i>positionOfO) new_data[i-1] = data[i];
            }
            data = new_data;
        }
    }
    ...
    public static  int[] mergeSort(int[] lst){
            ...
            public static  int[] merge(int[] leftList,int[] rightList){
                ...
                for (int i = 0; i < leftList.length; i++){
                    for (int j = count; j < lst.length; j++){
            ...
            public static void main(String[] args) {
                int[] elements = {1, 5, 10, 50, 100, 500, 1000, 5000, 10000, 20000, 40000, 60000, 80000, 100000,1000000,10000000};
                int inhalt = 3;
